...
export const appRouter = router({
  ...
  briefing: router({
    ...
    generateMock: protectedProcedure
      .mutation(async ({ ctx }) => {
        const userId = ctx.user.id;
        const { generateMockBriefing } = await import("./mock-data");
  ...
  notes: router({
    ...
    parseAndPreview: protectedProcedure
      ...
      .query(async ({ input }) => {
        const { parseNote, formatParsedDateTime, isDateInFuture } = await import("./note-parser");
    ...
    saveWithCalendarEvent: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.createEvent) {
          try {
            const { parseNote } = await import("./note-parser");
            const parsed = parseNote(input.content);
